...
const Conjunto: React.FC = () => {
    ...
    const [sortColumn, setSortColumn] = useState<keyof ConjuntoItem>('codigo');
    const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>('asc');
    const [currentPage, setCurrentPage] = useState(1);

    const itensIniciais: ConjuntoItem[] = [
        { id: 1, codigo: 'A01', familia: 'Installs v1', conjunto: 'Cables v1' },
        { id: 2, codigo: 'MCA', familia: 'Installs v2', conjunto: 'Cables v2' },
        // ... mais itens
    ];
    const itemsPerPage = 5;

    const handleSort = (column: keyof ConjuntoItem) => {
    ...
    const SortIcon = ({ column }: { column: keyof ConjuntoItem }) => {
    ...
    const filtrados = itensIniciais
        .filter(item =>
            ...
            (filtroConjunto === '' || item.conjunto.toLowerCase().includes(filtroConjunto.toLowerCase()))
        )
        .sort((a, b) => {
        ...
        <div className="bg-gray-50 min-h-screen p-6">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-gradient-to-r from-orange-500 to-orange-400 rounded-lg p-6 shadow-md mb-6 flex justify-between items-center">
                    <h1 className="text-2xl font-bold text-white">Conjuntos</h1>
                    <button className="bg-white text-orange-500 px-4 py-2 rounded-md shadow-sm flex items-center space-x-2 hover:bg-orange-50 transition">
                        <Plus size={18} />
                        <span className="font-medium">Novo Conjunto</span>
                    </button>
                </div>
