...
interface Entry {
  ...
  quantity: number;
  supplier: string;
  responsible: string; // Assumido como 'responsável pela entrada'
  entry_date: string; // ou Date
  origin?: string; // Adicionado para o filtro de origem
  // Adicione 'receive_responsible_field?: string;' se for um campo separado
}
...
const Entradas: React.FC = () => {
  ...
  const [inputResponsavelRecebimento, setInputResponsavelRecebimento] = useState(""); // Mantido, mas não usado na lógica de filtro atual sem campo correspondente
  ...
  const { data: allEntries, isLoading, refetch } = useQuery<Entry[], Error>({
  ...
  const filteredData = useMemo(() => {
    ...
    return allEntries.filter((entry) => {
      ...
      if (activeFilters.productName && !entry.equipment.name.toLowerCase().includes(activeFilters.productName.toLowerCase())) {
        isValid = false;
      }

      // Filtro por responsável pela entrada
      if (activeFilters.entryResponsible && !entry.responsible.toLowerCase().includes(activeFilters.entryResponsible.toLowerCase())) {
        isValid = false;
      }
  ...
  const handleSaveEntry = () => {
    handleClose();
    refetch(); // Refaz a busca de todas as entradas
  ...
  const columns = [
    ...
    // Poderia adicionar a coluna 'Origem' se quisesse exibi-la
    // { name: t('entry.origin_column_header', 'Origem'), accessor: (item: Entry) => item.origin || '-' },
  ];
  ...
  return (
      ...
      >
        <div className="max-w-7xl mx-auto">
          ...
          >
            {isLoading && <p className="text-center py-4">{t('entry.loading_entries')}</p>}
            {!isLoading && allEntries && <Table columns={columns} data={filteredData} />} {/* Passar filteredData para a tabela */}
            {!isLoading && (!allEntries || allEntries.length === 0) && <p className="text-center py-4">{t('entry.no_entries_found')}</p>}
            {!isLoading && allEntries && allEntries.length > 0 && filteredData.length === 0 && activeFilters && Object.keys(activeFilters).some(key => activeFilters[key as keyof EntryFilters]) && (
                <p className="text-center py-4">{t('entry.no_entries_match_filters', 'Nenhuma entrada corresponde aos filtros aplicados.')}</p>
            )}
          </motion.div>
