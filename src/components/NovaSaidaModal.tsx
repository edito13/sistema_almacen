...
const NovaSaidaModal: React.FC<NovaSaidaModalProps> = ({ isOpen, onClose, onSave }) => {
    ...
    const isFormValid = produto && conceitoSaida && quantidade && destino && dataSaida && dataEntrega && responsavelEntrega && responsavelRecebe;

    // Dados de exemplo para os selects (ajustar conforme necessário)
    const produtos = ["Monitor Dell 24\"", "Teclado Logitech", "Mouse Logitech", "Notebook HP", "SSD Kingston", "Cabo HDMI"];
    const conceitos = ["Uso Interno", "Transferência", "Devolução", "Venda", "Empréstimo"]; // Conceitos de Saída
    const responsaveis = ["Carlos Silva", "Maria Santos", "João Oliveira", "Ana Costa", "Pedro Almeida"];
    ...
    const renderInput = (label, type, value, onChange, placeholder, required = true, min = null, step = null, readOnly = false) => (
        <div className="space-y-1">
            <label className="block text-sm font-medium text-gray-700">
                {label} {required && <span className="text-red-500">*</span>}
            </label>
            <input
                ...
                value={value}
                onChange={onChange}
                className={`w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400 shadow-sm ${readOnly ? 'bg-gray-100 text-gray-600 cursor-not-allowed' : 'bg-white'}`}
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4 overflow-y-auto"
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl overflow-hidden"
                ...
                <div className="bg-gradient-to-r from-orange-500 to-orange-400 p-4 flex justify-between items-center"> {/* Cor do gradiente ajustada ligeiramente para combinar com a imagem */}
                ...
                <div className="p-6 overflow-y-auto" style={{maxHeight: "calc(100vh - 160px)"}}> {/* Aumentado padding para p-6 */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-5"> {/* Ajustado gap */}
                        {/* Coluna 1 */}
                        <div className="space-y-4">
                            {renderSelect("Produtos do Armazém", produto, e => setProduto(e.target.value), produtos)}
                            {renderInput("Destino", "text", destino, e => setDestino(e.target.value), "Local ou pessoa de destino")}
                            {renderSelect("Responsável pela entrega", responsavelEntrega, e => setResponsavelEntrega(e.target.value), responsaveis)}
                        </div>

                        {/* Coluna 2 */}
                        <div className="space-y-4">
                            {renderSelect("Conceito de Saída", conceitoSaida, e => setConceitoSaida(e.target.value), conceitos)}
                            {renderInput("Data de saída", "date", dataSaida, e => setDataSaida(e.target.value), "dd/mm/aaaa")}
                            {renderSelect("Responsável pelo recebimento", responsavelRecebe, e => setResponsavelRecebe(e.target.value), responsaveis)}
                        </div>

                        {/* Coluna 3 */}
                        <div className="space-y-4">
                            {renderInput("Quantidade", "number", quantidade, e => setQuantidade(e.target.value), "0", true, "1")}
                            {renderInput("Data de entrega", "date", dataEntrega, e => setDataEntrega(e.target.value), "dd/mm/aaaa")}
                            {renderInput("Registrado por usuário", "text", usuario, () => {}, "", false, null, null, true)} {/* Campo read-only */}
                        </div>
                    </div>
                ...
                {/* Rodapé com Botões */}
                <div className="p-4 bg-gray-50 border-t flex justify-end space-x-3"> {/* Botões alinhados à direita */}
                    <button
                        onClick={onClose}
