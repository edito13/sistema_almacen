...
const NovaEntradaModal: React.FC<NovaEntradaModalProps> = ({ isOpen, onClose, onSave }) => {
    ...
    const isFormValid = produto && conceito && quantidade && precoUnitario && dataEntrada &&
        paisFornecedor && comercioFornecedor && localizacao &&
        responsavelEntrada && responsavelRecebimento;

    const produtos = ["Monitor Dell 24\"", "Teclado Logitech", "Mouse Logitech", "Notebook HP", "SSD Kingston", "Cabo HDMI"];
    const conceitos = ["Nova compra", "Reposição", "Compra externa", "Doação", "Transferência"];
    const locais = ["Almoxarifado Principal", "Almoxarifado Secundário", "Escritório Central", "Área Técnica"];
    const paises = ["Angola", "Brasil", "Portugal", "EUA", "China", "África do Sul"];
    ...
    const renderInput = (label, type, value, onChange, placeholder, required = true, min = null, step = null, readOnly = false) => (
        <div className="space-y-1">
            ...
            <input
                ...
                value={value}
                onChange={onChange}
                className={`w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400 shadow-sm ${readOnly ? 'bg-gray-50' : 'bg-white'}`}
                ...
                <div className="p-5 overflow-y-auto" style={{maxHeight: "calc(100vh - 160px)"}}>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        ...
                        <div className="space-y-4">
                            {renderSelect("Produto", produto, e => setProduto(e.target.value), produtos)}
                            {renderInput("Quantidade", "number", quantidade, e => setQuantidade(e.target.value), "0", true, "1")}
                            {renderSelect("País do Fornecedor", paisFornecedor, e => setPaisFornecedor(e.target.value), paises)}
                            {renderSelect("Responsável pela Entrada", responsavelEntrada, e => setResponsavelEntrada(e.target.value), responsaveis)}
                        </div>

                        {/* Coluna 2 */}
                        <div className="space-y-4">
                            {renderSelect("Conceito", conceito, e => setConceito(e.target.value), conceitos)}
                            {renderInput("Preço Unitário", "number", precoUnitario, e => setPrecoUnitario(e.target.value), "0,00", true, "0", "0.01")}
                            {renderInput("Comércio do Fornecedor", "text", comercioFornecedor, e => setComercioFornecedor(e.target.value), "Nome do fornecedor")}
                            {renderSelect("Responsável pelo Recebimento", responsavelRecebimento, e => setResponsavelRecebimento(e.target.value), responsaveis)}
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            {renderSelect("Moeda", moeda, e => setMoeda(e.target.value), ["AOA (Kwanza)", "USD (Dolar)", "EUR (Euro)"])}
                            {renderSelect("Localização", localizacao, e => setLocalizacao(e.target.value), locais)}
                            {renderInput("Usuário", "text", "Admin", null, "", false, null, null, true)}
                        </div>
                    </div>
